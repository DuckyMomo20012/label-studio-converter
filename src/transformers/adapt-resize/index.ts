...
async function withTimeout<T>(
  ...
  return Promise.race([
    ...
    new Promise<T>(resolve =>
      setTimeout(resolve, timeoutMs, timeoutValue),
    ),
  ])
}
...
export async function adaptiveResizeBoundingBox(
  ...
  try {
    ...
    logger.warn(`Skipping adaptive resize for ${imagePath}: ${errorMessage}`)
    return points
  }
}
...
export const adaptResizeTransformer = (async (
  ...
  if (useAdaptiveThreshold) {
    logger.info('  Using adaptive thresholding based on image histogram')
  }
  ...
  const resized = await Promise.all(
    boxes.map(async (box, index) => {
      ...
      if (boxes.length > 10 && (index + 1) % 10 === 0) {
        logger.info(`  Processed ${index + 1}/${boxes.length} boxes`)
      }
  ...
  logger.info(`✓ Completed adaptive resize for ${boxes.length} boxes`)
  return resized
